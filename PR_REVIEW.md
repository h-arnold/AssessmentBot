...
- **First duplication pass:** extracted the repeated Slides parser test factories, module loading, mock installation, and harness setup into `tests/helpers/slidesParserTestHarness.js`, reusing the canonical `createMockABLogger` factory.
- **Second duplication pass:** centralised the repeated standard Slides parser suite lifecycle in `registerSlidesParserSuiteLifecycle` within `tests/helpers/slidesParserTestHarness.js`, and adopted it across all five migrated parser suites. The helper preserves per-suite global restoration (`DocumentParser` and `TaskDefinition` saved and restored per suite) and per-test ABLogger/SlidesApp installation and teardown; the table-failure suite retains its explicit `vi.restoreAllMocks()` cleanup through the `restoreMocks` opt-in.
- **Third duplication pass:** extracted the repeated SlidesAssignment/SheetsAssignment hermetic test-bed setup in `tests/assignment/slidesSheetsAssignmentCoverage.test.js` into `installAssignmentTestBed`. The helper preserves the title-specific Classroom course-work data supplied by each suite and each suite's unchanged `restoreGlobals()`/`vi.restoreAllMocks()` cleanup.
- **Verification:** the second-pass parser suites still pass (39 tests); the affected assignment suite passes (13 tests) and the full assignment suite passes (8 files / 116 tests); the full backend suite passes (169 files / 2,434 tests) with backend coverage still above the unchanged 85% thresholds; changed-file ESLint, Prettier, and `git diff --check` are clean; and the regression checker baseline run reports 0 regressions and 0 new failures while retaining the documented 6/8 baseline health.
- **Sonar status (confirmed):** after commit `1bd7be9`, SonarCloud reports a **PASS** quality gate for PR #304. `new_duplicated_lines_density` is 1.7141915279380253% (reported as 1.7%), below the ≤3% threshold, and all quality-gate conditions are OK, including SonarCloud Code Analysis. The earlier pre-push metric of 3.309% was superseded by this reanalysis.
