...
- **Verification:** the five migrated parser suites pass (39 tests); the full backend suite passes (169 files / 2,434 tests) with backend coverage still above the unchanged 85% thresholds; changed-file ESLint, Prettier, and `git diff --check` are clean; and the regression checker baseline run reports 0 regressions and 0 new failures while retaining the documented 6/8 baseline health.
- **Sonar status:** SonarCloud must reanalyse the pushed commit before the duplication percentage and quality gate can be confirmed. The quality gate is not yet confirmed.
