...
3. **Error (task titles unavailable)** — A data-completeness defect, reachable in v1: the warm-up `assignmentDefinitionPartials` dataset is missing the partial for the assignment's `definitionKey`, or a task has no `taskTitle`. `adaptMetricsToHeatmap` throws `TaskTitlesUnavailableError` (see `SPEC.md` error-state section). Handler: `TaskHeatmapPage` distinguishes this error from "assignment not found" and renders an **in-view Ant Design `Alert`** (`type="error"`) with the message "Task titles are currently unavailable. Please try reloading the page." The table region stays visible and **no** `onBack` auto-navigation occurs (the user must see the defect). This is distinct from "assignment not found", which silently returns to the overview.
