...
- **Current phase:** Sections 1–11, regression hardening, de-sloppification, and documentation sync complete (all reviewed clean, gates passed). Final standing regression session: `fix-301-stale-assignment-definitions-final` (created on the final committed tree; 7 checks pass and backend lint contains only the 10 accepted max-lines warnings). Section 10's full E2E gate had zero regressions and zero new failures after a one-worker rerun; the first full run exposed the already-observed `SelectWithAddNew` modal-entrance flake, which was reproduced historically and passed the focused rerun.
...
- **Section 10 baseline:** fresh session `fix-301-stale-assignment-definitions-post-s10`, created at `2026-09-19T10:51:32.596Z` after the Section 10 commit; report `.ts-regression-checker/reports/session-fix-301-stale-assignment-definitions-post-s10/baseline/baseline.txt`. Seven checks pass; backend lint alone fails on the same 10 accepted warnings (0 errors). The E2E check passes with the container PID limit raised to 2048.
- **Final baseline:** fresh session `fix-301-stale-assignment-definitions-final`, created at `2026-09-19T13:42:25.736Z` on the final committed tree; report `.ts-regression-checker/reports/session-fix-301-stale-assignment-definitions-final/baseline/baseline.txt`. Seven checks pass; backend lint alone reports the same 10 accepted warnings (0 errors). Playwright E2E, frontend/backend tests, builder tests, and builder compilation pass.
...
- **Section 4 complete.** Commit `2a420ad` feat: extend frontend upsert schema and error registry for definition recovery (branch `fix/301-stale-assignment-definitions`, pushed).
...
- Extracted feature-local orchestration in `src/frontend/src/features/classes/AssessTaskModal/`: `useAssessTaskFlow.ts` (467 lines; fetch/selection state, assessment + no-match machines, captured start context `{definitionKey, assignmentId, courseId}`, stub contract `assessmentRecoveryState: 'idle' | 'stale-prompt'` + `transitionToStaleRecovery(definitionKey)` that never selects `creating`, obsolete-completion guard, `@remarks`), composed with `useAssessTaskLinkFlow.ts` (317), `useAssessTaskCreateFlow.ts` (222), pure `assessTaskFlowData.ts` (153) and `assessTaskLinkPayload.ts` (43). `AssessTaskModal.tsx` 955→332 lines (declarative consumer; stacked wizard retained for create/choice paths). Guard identity source unified to `selectedAssignmentIdReference.current` across all three flows per green review. Suites: features/classes 278/278, full frontend 2067/2067, lint 0 errors/47 warnings (AssessTaskModal max-lines warning retired by the LOC gate), tsc clean. Shared-helper registry §9.23 entry 2 → implemented.
- **Regression-gate note:** gate failed round 1 on (a) e2e `classes-page-assess-task.spec.ts` asserting the old stale routing ("DEFINITION_STALE after link opens wizard…") — deterministic across retries; rewritten by the Playwright agent to assert the Section 6 routing-only contract (no create-wizard dialog; owner stays open; upsert/start ×1; positive recovery-prompt assertions deferred to Section 9) per the plan's assumption 4. (b) lint fingerprint changes: `AssessTaskModal.spec.tsx` 1672→1670 (rewritten assertions), e2e spec 1047→1069 (new routing assertions), retired `AssessTaskModal.tsx` 955 warning — all the same accepted warning class; handled per the established Section 5 pattern by refreshing the checkpoint baseline on the committed Section 6 tree: new standing session `fix-301-stale-assignment-definitions-post-s6` replaces `…-post-s5` for Sections 7–11.
- **Section 6 complete.** Commit `e3633b1` refactor: decompose AssessTaskModal into assessment orchestration hooks (branch `fix/301-stale-assignment-definitions`, pushed; SHA corrected in the evidence commit — initially recorded wrongly as b9d0813).
...
- Red phase: 12 tests in `src/frontend/src/features/assignmentWizard/assignmentWizardOrchestrator.spec.ts` (pinned surface via dynamic import: `resolveWizardEntryMode(intent)` four-mode taxonomy with recovery null-key fail-fast, `useAssignmentWizardOrchestrator({intent,onClose})` with intent union `{kind:'create'}|{kind:'update',definitionKey}|{kind:'explicit-reparse',definitionKey}|{kind:'recovery',definitionKey,onApprovalSuccess}` and return `{entryMode, blockingError, hasParsedTasks, reparseDefinition(), saveApproval(), cancelReview()}`; reparse schema-valid with `forceReparse: true` + existing key + no `taskWeightings`; save sends captured `expectedDefinitionUpdatedAt`; registry-driven `DEFINITION_STALE`/`DEFINITION_PARSE_FAILED` mapping proven by stubbing `mapErrorToUserMessage`; approval exactly-once; reparse-persists-immediately; update-load-failure never-create). Red review: one Improvement (hardcoded seed duplicating the canonical `editableDefinitions` record) fixed by consuming the committed fixture via Vite `?raw` import (precenteded by `src/index.css.spec.ts`); re-review CLEAN.
...
- Cleanup commit `743b617` — `refactor: reconcile wizard orchestration cleanup` — was pushed successfully on `fix/301-stale-assignment-definitions` (`e8eb7dc..743b617`). It includes the dead-hook removal, shared-builder deduplication, close-gating/test hardening, E2E naming cleanup, approval-baseline coverage, canonical documentation reconciliation, and this plan's verification notes.
...
- **Red phase:** Added `AssignmentDefinitionWizardModal.reparse.spec.tsx` with 9 focused tests covering update-only rendering, all enabling/disabling gates and visible disabled explanation, `forceReparse` request shape, busy/in-place refresh behaviour, and blocking parse failure treatment. Seven tests failed against the pre-change implementation as intended; two negative guards passed. Red review CLEAN.
- **Green phase:** Added the update-mode-only **Reparse documents** action to the stage-one document region. The action delegates to the existing orchestrator with `forceReparse: true` and the existing definition key, renders the existing blocking error treatment on failure, keeps the wizard in place while refreshing server-authoritative task rows/weights, and binds disabled-copy state to `properties.hasDirtyEdits`. Create mode and automatic/background parsing remain unchanged. Changed production files: `AssignmentDefinitionWizardModal.tsx`, `AssignmentDefinitionWizardModalShell.tsx`, `AssignmentDefinitionWizardReviewContent.tsx`, `assignmentWizardOrchestrator.ts`, and `useAssignmentDefinitionWizard.ts`. Green review CLEAN.
- **LOC/verification evidence:** assignment-wizard suite 87/87; full frontend suite 2079/2079; frontend lint 0 errors / 46 accepted warnings; TypeScript clean. The isolated Section 4 wizard E2E spec passed 6/6 with `--workers=1`.
- **Walkthrough:** MCP walkthrough covered enabled, disabled-with-explanation, busy, successful in-place refresh, and `DEFINITION_PARSE_FAILED` blocking failure states using the mandatory runtime mock mechanism and canonical editable-definition fixture. Confirmed one dialog, no create-path trigger, no automatic reparse, refreshed task rows, and no layout contradiction. Scratchpad-only walkthrough artifacts remain under `.opencode/scratchpad/walkthrough-mock/`.
- **Regression gate:** First full run under the previous container PID limit exposed the known `SelectWithAddNew` modal-entrance flake. With the container PID limit raised to 2048, the focused test passed 3/3 and the full regression rerun completed with 0 regressions and 0 new failures. The only remaining failure is the accepted backend lint `max-lines` debt (10 warnings); the E2E check passed.

### Commit evidence

- Commit `9682fcd` — `feat: add assignments-page reparse documents action` (6 files, +470/−2) on `fix/301-stale-assignment-definitions`; pre-commit hooks clean (Prettier, staged lint, builder tsc). `git push origin fix/301-stale-assignment-definitions` succeeded (`ba91d03..9682fcd`).
- Post-Section 10 baseline created as `fix-301-stale-assignment-definitions-post-s10`; seven checks pass and the only accepted debt is the existing backend lint `max-lines` warnings.
