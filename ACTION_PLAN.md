...
## **0 regressions.** The pre-existing `max-lines` on `tests/api/apiHandler.test.js` has been resolved by splitting into 8 files under `tests/api/apiHandler/` (all under 500 lines). 9 new E2E tests pass; 21 existing E2E tests pass; 858 frontend unit tests pass. Both outstanding follow-ups resolved (validation helper consolidation + canonical doc updates).
