...
> **Execution status:** Branch `feature/read-rehydrate-assignment`. **ACTIVE — Sections 1, 2, 3, 4 and 5 COMPLETE; next: Section 6 (regression) and Documentation/rollout.**
...
> **Current phase:** **Section 5 COMPLETE** — `tests/api/assignmentReadApi.test.js` rewritten so `installABClassControllerStub` exposes a `readRehydrateAssignment` spy (the dead `loadClass`/`rehydrateAssignment` spies removed); tests 7–11 rewired to the new delegation path, test 12 repurposed to assert `TypeError` from `readRehydrateAssignment` propagates verbatim; file header + helper JSDoc updated. Suite 15/15 green; Green code review PASS (7/7 acceptance criteria). Minor comment nitpicks (stale `normaliseDateFields`/`test 7d` references) fixed. No production code changed. **Next: Section 6 (regression and contract hardening) and Documentation/rollout.**
>
> **Progress log:**
>
