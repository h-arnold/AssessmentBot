...
> **Execution status:** Branch `feature/read-rehydrate-assignment`. **ACTIVE — Sections 1 and 2 COMPLETE; Section 3 (refactor `rehydrateAssignment`) next.**
...
> **Current phase:** **Section 2 COMPLETE** — Red tested, Green implemented (13/13 tests green: 12 ops + 1 facade), Green code review clean (PASS, only an out-of-scope stray-PNG merge-hygiene note, now reverted), Regression Gate passed (0 regressions, 0 new failures; the only failing check is the pre-existing `max-lines` debt accepted at baseline). **Next: Section 3 — refactor `rehydrateAssignment` to delegate to `readRehydrateAssignment`.**
>
> **Progress log:**
>
