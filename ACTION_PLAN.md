...
> **Execution status:** Branch `feature/read-rehydrate-assignment`. **ACTIVE — Sections 1, 2 and 3 COMPLETE; Section 4 (update `getAssignment_` handler) next.**
...
> **Current phase:** **Section 3 COMPLETE** — `rehydrateAssignment` refactored to delegate core loading to `readRehydrateAssignment`; all three validation guards, `courseId` extraction, `_replaceAssignmentInClass`, and the `try/catch` with `logger.error('rehydrateAssignment failed', …)` retained (19/19 rehydrateAssignment tests + 13/13 readRehydrateAssignment tests green); Green code review PASS-with-notes (double-log on error path ruled an acceptable consequence of the delegated design); Regression Gate passed (backend-lint unchanged at baseline 13 warnings; the lone `frontend-e2e` failure in `task-heatmap.spec.ts` re-ran green — a flake, not a feature regression). **Next: Section 4 — update `getAssignment_` transport handler.**
>
> **Progress log:**
>
