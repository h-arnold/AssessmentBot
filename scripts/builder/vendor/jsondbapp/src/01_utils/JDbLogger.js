/**
 * GASDBLogger - Provides standardised logging functionality for GAS DB
 *
 * Vendored upstream JsonDbApp logger implementation. Its console.* usage is
 * intentionally preserved for upstream/runtime parity in the committed
 * snapshot. This is a specific exception for this vendored file only, not a
 * general licence to use console.* elsewhere in active project code.
 */
class JDbLogger {
  /**
  ...
  // Preserve upstream console routing in this vendored file for runtime parity.
  /**
  ...
  static log(level, message, context = null) {
    ...
    if (levelValue !== undefined && JDbLogger.currentLevel >= levelValue) {
      ...
      if (levelValue === JDbLogger.LOG_LEVELS.ERROR) {
        console.error(formatted);
      } else if (levelValue === JDbLogger.LOG_LEVELS.WARN) {
        console.warn(formatted);
      } else {
        console.log(formatted);
      }
