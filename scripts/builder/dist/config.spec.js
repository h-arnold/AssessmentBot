...
const writeConfig = async (dirName, content) => {
...
const assertBuildStageError = async (run) => {
    let thrownError;
    try {
        await run();
    }
...
describe('loadBuilderConfig', () => {
    ...
    it('throws BuildStageError with stage "preflight-clean" when required fields are missing', async () => {
        const configPath = await writeConfig('missing-fields', JSON.stringify({ frontendDir: 'src/frontend' }));
        await expect(loadBuilderConfig(configPath)).rejects.toMatchObject({
            name: 'BuildStageError',
            stage: 'preflight-clean',
        });
...
describe('resolveBuilderPaths', () => {
    ...
    let repoRoot;
    let configPath;
    const writeBuilderConfig = async (config) => {
