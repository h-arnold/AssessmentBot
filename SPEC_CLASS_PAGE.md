...
13. **Both main sections use `Card` wrappers for visual consistency.** The `RecentAssignmentsSection` is wrapped in an Ant Design `Card` (`size="small"`, `title="Recent Assignments"`), matching the `StudentAveragesTableCard` pattern. The empty state (`Empty` with CTA button) renders inside the card body. Section heading content lives in the `Card` `title` prop rather than a separate `Typography.Title`. This is consistent with the existing codebase convention (e.g., `AssignmentsPage`, `ClassesPage`, `ClassesToolbar` all wrap content sections in `Card`).
...
- **`src/frontend/src/features/classPage/RecentAssignmentsSection.tsx`** (new) — presentational container. Renders the section `Card` (`size="small"`, `title="Recent Assignments"`) wrapping the row of cards (or the `Empty` state with CTA inside the card body). Owns no state; the page composition root owns the `onStartNewAssessment` callback.
...
5. **Recent Assignments section** — up to three cards, horizontally arranged, centre-aligned:
   - Each card title region: assignment name (rendered as the Ant Design `Card` `title` prop), then `Last Assessed: {date}` line. The literal "Recent Assignments" label is the section `Card` `title` above the row, not on every card. The "Last Assessed" line never renders a `—` placeholder; a missing `updatedAt` is a data bug that the adapter surfaces as a blocking state.
   ...
   - When zero assignments exist, render an Ant Design `Empty` inside the section `Card` body in place of the card row, with a description like `No recent assessments yet` and a primary `Start New Assessment` button below the message. The button opens the existing `AssessTaskModal` for the current class — the same handler as the header button. The section `Card` `title` `Recent Assignments` still renders above the empty state. The empty state is a positive nudge for new classes that have not been assessed yet.
...
- Renders the section `Card` (Ant Design `Card`, `size="small"`, `title="Recent Assignments"`) wrapping the row of up to three cards, or the empty state inside the card body.
- Owns no state. The page composition root owns the `onStartNewAssessment` callback.
- The empty state is an Ant Design `Empty` rendered inside the section `Card` body, with a description (`No recent assessments yet`) and a primary `Start New Assessment` button below the message that calls the `onStartNewAssessment` callback. The button is rendered as a child of the `Empty` component (via the `Empty` children slot) rather than via a non-existent `button` prop. The visual treatment is: `Empty description="No recent assessments yet"` with a `<Button type="primary" icon={<PlusOutlined />}>Start New Assessment</Button>` as the children.
- The section `Card` `title` (`Recent Assignments`) is rendered above both the row and the empty state (the empty state is positive-nudge copy, not a missing-data error).
...
- **Recent Assignments section — no assignments in the class.** Ant Design `Empty` rendered inside the section `Card` body, with description `No recent assessments yet` and a primary `Start New Assessment` button. The CTA opens the existing `AssessTaskModal` for the current class. The section `Card` `title` `Recent Assignments` still renders above the empty state.
...
- **`RecentAssignmentsSection.spec.tsx` (new, co-located)** — covers the section: the section `Card` wrapper with its title, the row of cards inside the card body, the empty state with the `Start New Assessment` CTA inside the card body, and the loading skeleton replacing the card body.
