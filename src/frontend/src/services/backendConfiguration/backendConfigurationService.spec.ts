...
describe('backendConfigurationService', () => {
  ...
  it('parses a valid masked configuration payload when loadError is omitted', async () => {
    ...
    await expect(getBackendConfig()).resolves.toEqual(validMaskedBackendConfigWithoutLoadError);
  });
  ...
  it('rejects invalid write payload shapes before transport', async () => {
    ...
    await expect(
      setBackendConfig(invalidWritePatch as unknown as Parameters<typeof setBackendConfig>[0])
  ...
  it('rejects write payloads with a malformed backendUrl before transport', async () => {
    ...
    await expect(setBackendConfig(invalidBackendUrlWritePatch)).rejects.toBeInstanceOf(ZodError);
    expect(callApiMock).not.toHaveBeenCalled();
  });
  ...
  it('rejects read-only write fields before transport', async () => {
    ...
    await expect(
      setBackendConfig(writePatchWithReadOnlyFields as Parameters<typeof setBackendConfig>[0])
    ).rejects.toBeInstanceOf(ZodError);
    expect(callApiMock).not.toHaveBeenCalled();
  });
});
