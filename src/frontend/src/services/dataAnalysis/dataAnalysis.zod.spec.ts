...
/**
 * Helper type for the input/filter schema module shape.
 */
interface InputSchemaModule {
  AnalysisFilterSchema: ParseOnly;
  AveragingAnalyserInputSchema: ParseOnly;
}

/**
 * Dynamically loads the dataAnalysis.zod module for input/filter suites.
 *
 * @returns {Promise<InputSchemaModule>} The imported module.
 */
async function loadDataAnalysisZod(): Promise<InputSchemaModule> {
  return import('./dataAnalysis.zod') as unknown as Promise<InputSchemaModule>;
}
