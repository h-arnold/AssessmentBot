...
const updateAssignmentTopicInput = {
  key: 'topic-algebra',
  record: {
...
const deleteAssignmentTopicInput = {
  key: 'topic-algebra',
};
...
describe('referenceDataService keyed contracts', () => {
  ...
  it.each([
    ...
    [
      ...
      { key: 'topic-algebra', name: 'Advanced Algebra', yearGroupKeys: ['yg-math', 'yg-advanced'] },
    ],
  ])(
    '%s() calls callApi with keyed input and parses keyed response',
  ...
  it.each([
    ...
    ['deleteAssignmentTopic', deleteAssignmentTopicInput],
  ])('%s() calls callApi and handles void response', async (methodName, input) => {
    const service = await loadReferenceDataService();
  ...
  it.each([
    ...
    ['deleteYearGroup', { key: '' }],
    ['createAssignmentTopic', { record: { name: '   ', yearGroupKeys: [] } }],
    ['updateAssignmentTopic', { key: '', record: { name: 'Algebra', yearGroupKeys: [] } }],
  ...
  ])('keyed methods reject malformed success payloads: %s()', async (methodName, response) => {
    callApiMock.mockResolvedValueOnce(response);
    const service = await loadReferenceDataService();
  ...
  describe('delete-blocked IN_USE transport propagation', () => {
    ...
    it('deleteAssignmentTopic() propagates an ApiTransportError with code IN_USE unchanged to callers', async () => {
      const inUseError = new ApiTransportError({
        requestId: 'req-delete-blocked-assignment-topic',
        error: {
          code: 'IN_USE',
          message: 'Assignment topic is referenced by one or more assignment definitions',
          retriable: false,
        },
      });
      callApiMock.mockRejectedValueOnce(inUseError);
      const { deleteAssignmentTopic } = await loadReferenceDataService();

      await expect(deleteAssignmentTopic(deleteAssignmentTopicInput)).rejects.toBe(inUseError);
    });
  });

  it('createAssignmentTopic() parses input with CreateAssignmentTopicInputSchema including yearGroupKeys', async () => {
    const response = {
      key: 'topic-algebra',
      name: 'Algebra',
      yearGroupKeys: ['yg-math'],
    ...
    callApiMock.mockResolvedValueOnce(response);
    const { createAssignmentTopic } = await loadReferenceDataService();

    await expect(createAssignmentTopic(createAssignmentTopicInput)).resolves.toEqual(response);
    expect(callApiMock).toHaveBeenCalledWith('createAssignmentTopic', createAssignmentTopicInput);
  });

  it('updateAssignmentTopic() parses input with UpdateAssignmentTopicInputSchema including yearGroupKeys', async () => {
    const response = {
      key: 'topic-algebra',
      name: 'Advanced Algebra',
      yearGroupKeys: ['yg-math', 'yg-advanced'],
    ...
    callApiMock.mockResolvedValueOnce(response);
    const { updateAssignmentTopic } = await loadReferenceDataService();

    await expect(updateAssignmentTopic(updateAssignmentTopicInput)).resolves.toEqual(response);
    expect(callApiMock).toHaveBeenCalledWith('updateAssignmentTopic', updateAssignmentTopicInput);
  });

  it('deleteAssignmentTopic() sends the assignment-topic key', async () => {
    const { deleteAssignmentTopic } = await loadReferenceDataService();

    await deleteAssignmentTopic(deleteAssignmentTopicInput);
    expect(callApiMock).toHaveBeenCalledWith('deleteAssignmentTopic', deleteAssignmentTopicInput);
  });
});
