...
describe('GoogleClassroomAssignmentsResponseSchema', () => {
  it('accepts an assignment with topicId and topicName as strings', () => {
    expect(
      GoogleClassroomAssignmentsResponseSchema.parse([
        { assignmentId: 'a1', title: 'Essay', topicId: 'abc123', topicName: 'Algebra' },
      ])
    ).toEqual([{ assignmentId: 'a1', title: 'Essay', topicId: 'abc123', topicName: 'Algebra' }]);
  });

  it('accepts an assignment with topicId and topicName as null', () => {
    expect(
      GoogleClassroomAssignmentsResponseSchema.parse([
        { assignmentId: 'a1', title: 'Essay', topicId: null, topicName: null },
      ])
    ).toEqual([{ assignmentId: 'a1', title: 'Essay', topicId: null, topicName: null }]);
  });

  it('defaults missing topicId and topicName to null', () => {
    expect(
