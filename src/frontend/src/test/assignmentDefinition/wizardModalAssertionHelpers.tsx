 ...
 *
 * Owns the shared `assert*` helpers plus the form-state value getters, keeping the
 * element-query and interaction surface in `wizardModalInteractionHelpers`.
 */
...
import {
  ...
  type ModalElementQueries,
} from './wizardModalInteractionHelpers';
...
/**
 * Asserts that both document URL inputs remain editable.
 *
 * @param {HTMLElement | ModalElementQueries} container The modal or container object.
 * @returns {Promise<void>} Completion signal.
 */
export async function assertDocumentUrlFieldsEnabled(
  container: HTMLElement | ModalElementQueries
): Promise<void> {
  const modal = 'modal' in container ? container.modal : container;
...
/**
 * Asserts that every control conflicting with a pending document change or an
 * in-flight mutation is locked: title, topic, year group, assignment weighting and
 * each per-task weighting.
 *
 * @param {HTMLElement | ModalElementQueries} container The modal or container object.
 * @returns {Promise<void>} Completion signal.
 */
export async function assertConflictingControlsDisabled(
  container: HTMLElement | ModalElementQueries
): Promise<void> {
  const modal = 'modal' in container ? container.modal : container;
  ...
  await waitFor(() => {
    ...
    expect(yearGroupSelect).toBeDisabled();
    expect(getAssignmentWeightingInput({ modal })).toBeDisabled();
...
/**
 * Asserts that every control conflicting with a pending document change or an
 * in-flight mutation is editable again.
 *
 * @param {HTMLElement | ModalElementQueries} container The modal or container object.
 * @returns {Promise<void>} Completion signal.
 */
export async function assertConflictingControlsEnabled(
  container: HTMLElement | ModalElementQueries
): Promise<void> {
  const modal = 'modal' in container ? container.modal : container;
  ...
  await waitFor(() => {
    ...
    expect(yearGroupSelect).toBeEnabled();
    expect(getAssignmentWeightingInput({ modal })).toBeEnabled();
    getTaskWeightingInputs({ modal }).forEach((input) => {
