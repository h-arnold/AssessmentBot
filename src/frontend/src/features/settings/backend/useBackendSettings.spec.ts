...
vi.mock('../../../services/backendConfiguration/backendConfigurationService', () => ({
...
describe('useBackendSettings', () => {
  ...
  it('resolves the initial backend config into editable state after the first read', async () => {
    ...
    expect(mapBackendConfigToBackendSettingsFormValuesMock).toHaveBeenCalledWith(baseBackendConfig);
  });
  ...
  it('ignores additional save requests while the first save is still in flight', async () => {
    ...
    expect(mapBackendSettingsFormValuesToBackendConfigWriteInputMock).toHaveBeenCalledTimes(1);
