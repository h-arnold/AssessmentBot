...
class OverviewSheetManager extends BaseSheetManager {
  ...
  fetchData() {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          'After cleaning invalid references, batchGet still fails. Please check references manually.'
        );
      }
    }
  }
  ...
  calculateAverages() {
    ...
    for (const name in aggregatedData) {
      ...
      const completenessAvg =
        data.completenessCount > 0
          ? (data.completeness / data.completenessCount).toFixed(2)
          : 'N/A';

      const accuracyAvg =
        data.accuracyCount > 0 ? (data.accuracy / data.accuracyCount).toFixed(2) : 'N/A';

      const spagAvg = data.spagCount > 0 ? (data.spag / data.spagCount).toFixed(2) : 'N/A';
  ...
  buildRequests() {
    ...
    // Format headers
    this.requests.push(
      ...this.createHeaderFormattingRequest(sheetId, 0, 1, {}, 0, this.headers.length)
    );
