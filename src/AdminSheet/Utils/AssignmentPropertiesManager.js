...
class AssignmentPropertiesManager {
  ...
  static getDocumentTypeById(documentId) {
    ...
    try {
      ...
      progressTracker.logAndThrowError(
        `Failed to determine document type for ID ${documentId}. Ensure the ID is correct and you have permissions.`,
        e
      );
    }
  }
  ...
  static saveDocumentIdsForAssignment(assignmentTitle, documentIds) {
    ...
    if (documentIds.referenceDocumentId === documentIds.templateDocumentId) {
      progressTracker.logAndThrowError(
        'Reference document ID and template document ID cannot be the same.'
      );
    }
  ...
  static getDocumentIdsForAssignment(assignmentTitle) {
    ...
    if (value) {
      try {
        ...
        const progressTracker = ProgressTracker.getInstance();
        progressTracker.captureError(
          e,
          `Error parsing properties for assignment "${assignmentTitle}"`
        );
  ...
  static migrateEmptyTaskToTemplateTask() {
    ...
    progressTracker.logInfo(
      `AssignmentPropertiesManager: Migrated ${migratedCount} assignment(s) from 'emptySlideId' to 'templateSlideId'.`
    );
  }
}
