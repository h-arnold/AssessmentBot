...
class UpdateManager extends BaseUpdateAndInit {
  ...
  ensurePropertiesStoreTransferred(oldAdminSheetId, newAdminSheetId) {
    ...
    if (newPropertiesStore) {
      ...
      ABLogger.getInstance().info(
        'No propertiesStore in new admin sheet template, sheet should have been copied by cloneSheets'
      );
    }
  }
   ...
   * 3. Ensuring the propertiesStore sheet from the old admin sheet is preserved.
   * 4. Archiving the old version.
   * 5. Opening the new sheet in a browser window.
  ...
  updateAdminSheet() {
    ...
    // Ensure propertiesStore from old admin sheet is in the new one
    this.ensurePropertiesStoreTransferred(currentAdminSheetFileId, newSheetId);
