...
description: Orchestrates the other agents to implement an action plan. Coordinates delivery against ACTION_PLAN.md with strict, sequential, TDD-first workflow.
license: MIT
user-invocable: true
allowed-tools:
  - read_file
...
**IMPORTANT NOTE:** If the action plan allows, **and only if it allows**, you may deviate from this exact loop where it is inappropriate, for example if you are completing a refactor or cleanup where strict adherence to TDD would not make sense.
