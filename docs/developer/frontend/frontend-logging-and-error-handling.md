...
#### Transport- and service-boundary logging (`callApi`, service-layer schema validation)

The API transport boundary (`src/frontend/src/services/apiService.ts`) is the single choke point for all backend calls, so its logs must make the failing call identifiable without manual per-feature wiring. The same contract extends to service-layer schema validation: when `callApi` returns successfully but the inner response fails its service-level Zod schema (for example `assignmentAssessmentService.parseResponse` validating `AssignmentFullResponseSchema`), the rejection must also carry the structured diagnostics rather than a bare `ZodError`. Apply these rules at both boundaries:
...
- always include the backend `method` (e.g. `getAssignment`) in failure `metadata` so errors are grep-able by endpoint, while keeping `context` at the stable `services/apiService.callApi` (transport) or `services/<module>.parseResponse` (service-layer validation).
