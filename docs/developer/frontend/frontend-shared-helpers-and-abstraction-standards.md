...
### 9.21 Backend settings `helperText` descriptor extension (planned)

This entry records the planned descriptor-type extension for the Auth Service feature (source: repository-root `SPEC.md` and `ACTION_PLAN.md`). It is recorded here so the implementation agent has a documented target contract. The implementation agent removes the `Not implemented` marker as the entry is delivered.
