...
- Shared Forename/Surname Ant Design columns: `buildStudentNameColumns<Row>(options)` at `src/frontend/src/features/shared/studentNameTableColumns.tsx` — the column counterpart consumed by the two tables above; see §9.18 item 8.
...
- Call-site rationale: maps the data analysis service's `MetricResult` discriminated union to a `{ color, cellStyle, muted }` resolution that the Ant Design `Tag` (and table cells) consume; user-facing text is produced separately by the shared `formatMetricDisplayText` formatter (see §9.18 item 7). The shared union has four states (`computed`, `notAttempted`, `excluded`, `error`); task-level display shapes use the narrower, exported `TaskDisplayMetric` union (`computed`, `notAttempted`, `error`, derived from `TaskDisplayMetricSchema` in `dataAnalysis.zod.ts`), which omits the aggregate-only `excluded`. For `computed` values the colour is a **continuous gradient**: the normalised position `t = (value - lower) / (upper - lower)` (clamped to `[0, 1]`) maps to an `hsl` colour whose hue sweeps red (`0`) → amber (`60`) → green (`120`), with lightness darker at the range ends (darkest red at the floor, darkest green at the ceiling) and lighter in the middle. `cellStyle` carries the matching light-background / dark-text pair for table cells. `notAttempted` returns `'#434343'` (dark grey with light grey cell background), `error` returns the `errorColor` token (default `'volcano'`), and `excluded` returns the neutral `default` colour plus a `cellStyle` carrying the theme-aware `metric-tone-excluded-cell` stylesheet class defined in `src/frontend/src/index.css`. The discrete states are not uniform: `notAttempted` and `error` supply preset inline `cellStyle` properties, whereas `excluded` supplies only the stylesheet `className` (the `'default'` key in `METRIC_TONE_CELL_STYLE` is an unused fallback, not the excluded treatment). Pure function; the only React import is the type-only `CSSProperties`, so there is no React runtime or antd dependency. Validates `range.upper > range.lower` and throws on violation.
- Status: `Implemented`
- Implementation notes:
  ...
  - `MetricToneColor = 'red' | 'gold' | 'green' | 'default' | 'volcano'` is the union for the discrete Tag colours: `excluded` returns the neutral `'default'` Tag colour with the theme-aware `metric-tone-excluded-cell` stylesheet class as its cell style, and `error` returns `errorColor` (default `'volcano'`). `notAttempted` returns dark grey `'#434343'` with its own light-grey cell background (not a `MetricToneColor` member). The column **score-range filter** (see helper 4) is the consumer of filter values, not the continuous gradient. Any future revision to `MetricToneColor` is a cross-spec breaking change.
  ...
  - File size is under the 550-line threshold; no separation needed.
...
- Call-site rationale: renders a single `MetricResult` as a coloured Ant Design `Tag` using the output of `resolveMetricTone`. Exposes `precision` (default 2), `emphasised` (default false), `range` (pass-through), and `errorColor` (pass-through; no `MetricPill`-level default — the default lives in `resolveMetricTone`). Consumed by `RecentAssignmentCard` (four instances per card) and by each metric header of the task-preview card (`features/taskHeatmap/TaskPreviewCard.tsx`, rendered as `<MetricPill metric={...} precision={0} compact />`). It is **not** used by the Student Averages table: `features/classPage/studentAveragesTableColumns.tsx` renders the score as plain text via `formatMetricDisplayText` and applies `resolveMetricTone(...).cellStyle` directly through `onCell`. Future consumers: cohort, trend, and distribution analyses.
- Status: `Implemented`
- Implementation notes:
...
- Status: `Implemented`
- Implementation notes:
  - Folder created at the planned path. Current contents (16 files): `MetricPill.tsx`, `MetricPill.spec.tsx`, `metricComparator.ts`, `metricComparator.spec.ts`, `metricDisplayMeta.ts`, `metricDisplayText.ts`, `metricDisplayText.spec.ts`, `metricRangeFilter.tsx`, `metricRangeFilterDropdown.tsx`, `metricRangeFilterDropdown.spec.tsx`, `metricRangeKey.ts`, `metricRangeKey.spec.ts`, `metricStateRank.ts`, `metricStateRank.spec.ts`, `metricTone.ts`, `metricTone.spec.ts`.
...
- Status: `Implemented`
- Implementation notes:
  - Exports: `METRIC_STATE_RANK_ASC` (`asc`: computed → notAttempted → excluded → error), `METRIC_STATE_RANK_DESC` (`desc`: error → excluded → notAttempted → computed), and `getMetricStateRank(metric, direction)`, which throws on an unknown state rather than falling back to a rank. Within the computed band, value ordering and the `studentId` tie-break are applied by the shared `compareMetricsByStateRank` comparator (see entry 7).
...
- Call-site rationale: single source of truth for app-wide locale-aware student-name ordering, structurally re-typed onto `Readonly<{ studentName: string; studentId: string }>` so no feature-owned type enters the services layer and every consumer row shape satisfies it without casts. Consumers: the Class page adapter (`classPageAdapter.ts`, initial `studentAverages` pre-sort) and view-model builder (`classPageModel.ts`, default and cleared full-name ascending), and the Task Heatmap table (`TaskHeatmapTable.tsx`, `dataSource` pre-sort). Derived Forename/Surname column sorting uses the separate `compareStudentNamePart` comparator from `utils/splitStudentName.ts` (see §3.6), not `compareStudentNames`. The previous `compareHeatmapStudentName` wrapper (and `features/taskHeatmap/taskHeatmapModel.ts`) was removed during the Heatmaps review as a zero-value pass-through. Flat placement follows `src/frontend/AGENTS.md` §14 (single-file services stay flat).
- Status: `Implemented`
- Implementation notes:
...
- Implementation notes:
  ...
  - Co-located spec: `metricComparator.spec.ts` (state-rank ordering in both directions, computed-band value ordering, direction-neutral row-id tie-break, unknown-state throw).

### 9.18 Issue #307 shared extensions
...
- Owning modules: `metricTone.ts` (owns `EXCLUDED_METRIC_ACCESSIBLE_LABEL`;
  its `excluded` cell style carries the theme-aware
  `metric-tone-excluded-cell` stylesheet class defined in
  `src/frontend/src/index.css`), `metricDisplayText.ts` (the shared
  `formatMetricDisplayText` formatter — see item 7), `MetricPill.tsx` (renders
  the **Excluded** label with `role="img"` and the shared accessible name),
...
7. Helper: `formatMetricDisplayText(metric, precision)` — shared metric display
   formatter

- Decision: **new shared helper**.
- Owning module/path:
  `src/frontend/src/services/dataAnalysis/metricDisplay/metricDisplayText.ts`.
- Call-site rationale: the heatmap cell renderer, the Student Averages columns,
  and `MetricPill` previously each formatted a metric independently, and the
  heatmap fallback mislabelled the aggregate-only `excluded` value as `E`. One
  formatter now maps every shared state to its display text (`computed` at the
  requested precision, `N`, **Excluded**, `E`) and throws on an unknown state.
  It accepts the structural `{ state, value }` projection, so the narrower
  task-display union can reuse it without a feature dependency.
- Status: **Implemented**.

8. Helper: `buildStudentNameColumns<Row>(options)` — shared Forename/Surname
   table columns

- Decision: **new cross-feature presentational helper** (see also §3.6).
- Owning module/path:
  `src/frontend/src/features/shared/studentNameTableColumns.tsx`.
- Call-site rationale: the Class page Student Averages table and the Task
  Heatmap table both render the same split student-name pair. One builder owns
  the Ant Design column shape — renderers via the shared `splitStudentName`
  helper, sorters via `compareStudentNamePart`, and the shared
  `APP_COL_WIDTH_FORENAME` / `APP_COL_WIDTH_SURNAME` widths — while each table
  keeps its own `sticky` behaviour and initial full-name ordering. The pure
  splitting and comparison functions remain in `utils/splitStudentName.ts`.
- Status: **Implemented**.
...
#### 9.19.1 Pure adapter: `classPageAdapter`
...
#### 9.19.2 Pure view-model builder: `classPageModel`
...
#### 9.19.3 Data orchestrator hook: `useClassPageData`
...
#### 9.19.4 Page composition root: `ClassPage`
...
#### 9.19.6 Presentational components
...
- Status: `Implemented`
- Implementation notes: Pure function exporting `buildStudentAveragesTableColumns(filters)` — no React hooks. Six columns: `forename` and `surname` (built by the shared `buildStudentNameColumns` helper — see §9.18 item 8 — which renders via `splitStudentName`, sorts via `compareStudentNamePart`, and uses the shared width tokens; no `sticky` here), then `completeness`/`accuracy`/`spag`/`average` (metric columns with a numeric **score-range filter** via `buildMetricRangeFilter`, gradient cell colour via `resolveMetricTone(...).cellStyle`, plain-text score render). The `filters` prop is `StudentAveragesTableFilters` (`readonly string[]` encoded filter keys per column).

#### 9.19.7 Zod trust-boundary schema: `classPageAdapter.zod`
...
#### 9.19.8 Generic duplicate-detection helper extracted from duplicate validators
...
- Decision: `reuse`
- Owning module/path: `src/frontend/src/features/taskHeatmap/TaskHeatmapTable.tsx` (consumes `services/dataAnalysis/compareStudentNames.ts` directly for its default order; the previous `compareHeatmapStudentName` wrapper in `features/taskHeatmap/taskHeatmapModel.ts` was deleted during the Heatmaps review as a zero-value pass-through)
- Call-site rationale: the heatmap table's initial row order uses the canonical shared `compareStudentNames` comparator directly, with no intermediate wrapper. The derived Forename/Surname column sorters use the separate `compareStudentNamePart` comparator from the shared `utils/splitStudentName.ts` module.
- Status: `Implemented`
- Implementation notes:
  ...
  - The `dataSource` pre-sort via `rows.toSorted(compareStudentNames)` uses the shared comparator directly (direction-neutral; the heatmap always sorts ascending). Neither split column sets `defaultSortOrder`, so the full-name pre-sort remains the initial order until a user sorts.
...
#### 9.19.12 Heatmap table component: `TaskHeatmapTable`

15. Component: `TaskHeatmapTable` — presentational heatmap table (grouped headers, score-range filters, sorters)

- Decision: `keep local`
- Owning module/path: `src/frontend/src/features/taskHeatmap/TaskHeatmapTable.tsx` (moved from `features/classPage/` by the TaskHeatmap extraction; the single student-name column was later split into sticky Forename/Surname columns built via the shared `buildStudentNameColumns` helper — see §9.18 item 8)
- Call-site rationale: pure presentational table built from a `TaskHeatmapData` view model. The Ant Design `Table<TaskHeatmapRow>` column tree is assembled inside a component-level `useMemo`: two sticky top-level columns produced by the shared `buildStudentNameColumns` helper (`fixed: 'start'`, shared `APP_COL_WIDTH_FORENAME`/`APP_COL_WIDTH_SURNAME` widths, renderers via `splitStudentName`, sorters via `compareStudentNamePart`). Neither column sets `defaultSortOrder`; the initial full-name ascending order comes from a `rows.toSorted(compareStudentNames)` pre-sort. Then one grouped column per `taskColumn` (title = `taskColumn.taskTitle ?? taskColumn.taskId`) with `Completeness`/`Accuracy`/`SPaG` children produced by `buildTaskMetricSubColumns`. Each metric sub-column uses `buildMetricRangeFilter` (a numeric score-range `filterDropdown` + `onFilter` over a two-thumb `Slider` bounded by the fixed default tone range `0–5`), a canonically ordered `sorter` delegating through the local `buildMetricSorter` wrappers to the shared `compareMetricsByStateRank` (state rank computed → `notAttempted` → `excluded` → `error`, numeric value ascending within the computed band, ascending `studentId` tie-break; the heatmap always sorts ascending), and renders the formatted score (`formatMetricDisplayText`: integer for computed scores, `N` for `notAttempted`, `E` for `error`) inside a gradient-coloured cell (`resolveMetricTone(...).cellStyle` applied via `onCell`) whose `aria-label` keeps the full stored name string in the form `"[Student Name], [Task Title or Task ID fallback], [Metric]: [Score]"` (the label uses `taskColumn.taskTitle ?? taskColumn.taskId`). Each score sits in a keyboard-operable `role="button"` span (Enter/Space activation) that opens a `Popover` whose content is deferred to `DeferredPopoverContent` (loading `<output>` skeleton while the assignment query is pending → error `Alert` → `TaskPreviewCard`). `pagination={{ pageSize: 50, showSizeChanger: true }}`, `bordered`, `scroll={{ x: 'max-content' }}`, `aria-label="Task Heatmap"`.
- Status: `Implemented`
- Implementation notes:
  ...
  - Empty-state: a "No submissions yet" caption renders above the table only when `taskColumns.length > 0 && rows.length > 0 &&` every cell is `notAttempted`; the guard suppresses the caption for the zero-tasks variant (`taskColumns: []` → only the Forename and Surname columns render).
  ...
  - Co-located spec: `TaskHeatmapTable.spec.tsx` (grouped headers, the score-range filter dropdown, Forename/Surname sorting via `compareStudentNamePart`, the preserved full-name default order, split-cell rendering, per-cell full-name `aria-label` format, no-submissions caption, zero-tasks variant, and the cell-popover suite — popover wrapping, `TaskPreviewCard` content, preserved cell tone and aria-labels after popover integration, loading skeleton vs error `Alert` states, and populated/empty cell-preview lookup cases).
...
- Status: `Implemented` (`task-heatmap.spec.ts` plus the split `task-heatmap-end-to-end-helpers.ts` / `task-heatmap-fixtures.ts` helpers; 11 passing tests).
- Implementation notes:
...
#### 9.19.15 ClassPage assignment prefetch support helpers

These entries record the helpers introduced to support the ClassPage assignment prefetch feature. All entries delivered in the ClassPage Assignment Prefetch cycle.
...
#### 9.19.16 Task Preview Card real-data wiring helpers
...
24. Helper: `assembleTaskPreviewData(cellData, metricResult, metricKey, taskId): TaskPreviewData` — maps a `CellPreviewData` (or `null`) and the analyser's `TaskDisplayMetric` into the `TaskPreviewCard`'s `TaskPreviewData` contract

- Decision: `new`
- Owning module/path: `src/frontend/src/features/taskHeatmap/assembleTaskPreviewData.ts`
- Call-site rationale: the single point where the wider backend types (`SPREADSHEET`, `base`, `unknown` content) are narrowed to `TaskPreviewData`'s narrower contract; handles artifact type coercion, reasoning extraction per metric key, and pass-through of the analyser's `TaskDisplayMetric` on the single `metric` field with no score/state reassembly. When `cellData` is `null` — no submission exists for the `(student, task)` pair — it substitutes the shared frozen `NOT_ATTEMPTED_METRIC` from `heatmapAdapter.ts` even when the analyser reported `computed`, so a computed metric is never shown for an absent submission. The shape contract is recorded in `docs/developer/data-shapes/frontend-data-analysis-response.md`. Has exactly one caller (`TaskHeatmapTable` cell render).
- Status: `Implemented`
- Implementation notes: Maps an AssignmentFull cell to `TaskPreviewData` (reasoning, artifact type, and the analyser's `metric` forwarded unchanged); a `null` `cellData` returns the empty no-submission defaults (`TEXT`, empty content, empty reasoning) with `NOT_ATTEMPTED_METRIC` in place of the analyser metric.
...
- Status: `Implemented`
- Implementation notes:
  - Implemented alongside the `lastUpdated` → `updatedAt` rename.
...
### 9.21 Data analysis accumulator helpers
...
- Status: `Implemented` (`adaptMetricsToHeatmap` added to `heatmapAdapter.ts`; `HeatmapResult`/`HeatmapRow`/`HeatmapCell`/`HeatmapTaskColumn` interfaces and the frozen `NOT_ATTEMPTED_METRIC` task-display fallback are exported — the fallback is shared by the `buildCellsForStudent` cell projection and the task-preview no-submission branch, see §9.19.16 entry 24).

### 9.22 Backend settings `helperText` descriptor extension

This entry records the descriptor-type extension for the Auth Service feature.
...
## 9.23 Heatmaps builder surface helpers
...
- Relevant canonical doc target: this section (§9.23).
...
## 9.24 Stale-definition recovery orchestration helpers (issue #301)

Entries for stale assignment-definition recovery. The delivered items are the assessment orchestration module (entry 2), the chrome-free review-content component (entry 3), the form-state module (entry 4), and the feature-local discard-confirmation component (entry 5). The planned generic wizard orchestrator hook and its entry-intent taxonomy were not retained (entry 1): once the assessment-side recovery flow owned stale recovery, the orchestrator hook had no live consumer, so the cleanup removed `resolveWizardEntryMode`, `useAssignmentWizardOrchestrator` and the generic `{kind: ...}` recovery entry intent. The delivered entries below reflect the current implementation.

1. Wizard orchestrator module — not retained as a standalone orchestration hook. The live create/update wizard remains owned end-to-end by `useAssignmentDefinitionWizard`, which composes the pure form-state derivations (`assignmentWizardFormState`), the form-initialization/baseline hook (`assignmentWizardFormInitialization`), the shared mutation/error-mapping sequence (`assignmentWizardMutation`) and the close/dismissal predicates (`assignmentWizardDismissal`). Stale recovery is owned separately by `useAssessTaskRecoveryFlow` (entry 2 covers the assessment-side orchestration). The only helper retained in `assignmentWizardOrchestrator.ts` is `buildReparseRequest`.

- Decision: `superseded` (the generic orchestrator hook and entry-intent surface were removed; the shared forced-reparse request builder was retained)
- Owning module/path: `src/frontend/src/features/assignmentWizard/assignmentWizardOrchestrator.ts` (exports `buildReparseRequest` only)
- Call-site rationale: `buildReparseRequest` keeps the forced-reparse payload in one place. It is consumed by both the Assignments-page update wizard (`useAssignmentDefinitionWizard.handleReparseDocuments`) and the recovery flow (`useAssessTaskRecoveryFlow.reparseLoadedDefinition`), so both send the same ID-shaped `forceReparse: true` request with no weighting patch and let the backend reconcile the stored task weightings.
- Status: `Superseded`
...
- Call-site rationale: Decomposition required by the frontend standards; moves both state machines and the API flows out of the rendering component per §3.2 of `src/frontend/AGENTS.md`. Exposes the recovery state (`assessmentRecoveryState: 'idle' | 'stale-prompt'`) with the `endRecovery` and `settleAssessment` handlers; the internal `transitionToStaleRecovery(definitionKey)` — invalidates the definition-partials cache, clears the no-match resolution, captures the stale key and never selects `creating` — is driven by matched/link `DEFINITION_STALE` rejections and is not returned to consumers. It also applies an obsolete-completion guard so late assessment-start/upsert completions cannot act on a different assignment after close/reopen or selection change. The link flow (`useAssessTaskLinkFlow.ts`) and create flow (`useAssessTaskCreateFlow.ts`) are composed feature-local hooks sharing a documented host contract; pure flow contracts, cache validation and derivations live in `assessTaskFlowData.ts`, with link-payload derivation in `assessTaskLinkPayload.ts` — each module below the 500-line gate.
- Status: `Implemented`

3. Chrome-free review-content component and wizard entry-contract extension. The review-content component is `Implemented` at `src/frontend/src/features/assignmentWizard/AssignmentDefinitionWizardReviewContent.tsx`, and is recorded simultaneously in `docs/developer/frontend/frontend-modal-patterns.md` §3.4. It extracts the shell's body form and footer content from `AssignmentDefinitionWizardModalShell`'s `Modal` chrome so recovery and in-modal create can render wizard content inside a single owning modal. The footer rendering now lives in its own responsibility-coherent module, `AssignmentDefinitionWizardReviewFooter.tsx`, re-exported from the review-content module so the chrome-free content and the full-shell modal share one footer definition. The planned explicit recovery entry intent on the wizard entry contract was not retained: stale recovery is owned directly by `useAssessTaskRecoveryFlow`, which reuses the form-state module and `buildReparseRequest` rather than routing through a wizard entry mode.

4. Wizard form-state module — pure form-state derivation (form hydration, dirty-state rules, document-change state, task-row building) at `src/frontend/src/features/assignmentWizard/assignmentWizardFormState.ts`. Status: `Implemented`. Extracted from `useAssignmentDefinitionWizard.ts` with no behavioural change; consumed by the create/update wizard (`useAssignmentDefinitionWizard`) and by the assessment-side recovery flow (`useAssessTaskRecoveryFlow`, which reuses `hydrateFormFromDefinition`, the topic/year-group option builders, `calculateDirtyState` and the `TaskRow`/`DocumentChangeState` contracts). Rationale: deterministically testable state derivation shared by both flows.

5. Discard-confirmation component — feature-local component shared by the assignment-definition wizard, the in-modal create review, and the stale-recovery review surface.

- Decision: `new` (feature-local extraction)
- Owning module/path: `src/frontend/src/features/assignmentWizard/AssignmentDiscardConfirm.tsx` (feature-local)
...
- **First entry:** `dateFormatting.ts` — exports `formatUpdatedAtLabel(updatedAt: string | null): string`. See §9.20 item 1 above for the full contract.
