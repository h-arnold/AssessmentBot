...
Entries for stale assignment-definition recovery. Status: all `Implemented` — the Section 5 form-state module and chrome-free review-content component have landed (entries 3–4), as have the Section 6 assessment orchestration module (entry 2) and the Section 7 wizard orchestrator module and recovery entry intent (entries 1 and 3). Recorded before implementation started per the SPEC.md handoff gate; each implementing section moved its entries to `Implemented` as it landed.

1. Wizard orchestrator module — dedicated, readable orchestration file under `src/frontend/src/features/assignmentWizard/` owning the wizard process end-to-end: entry modes (create, update, explicit reparse, recovery), the parse → review → save sequence, mutation calls, error mapping, and stage transitions. It loads the existing definition with a fresh `fetchQuery` on reparse but performs no query invalidation of its own: mutation-driven cache invalidation stays owned by the shared mutation sequence/hook plumbing (`useWizardMutationSequence` in `assignmentWizardMutation.ts`, composed over `useWizardUpsertMutation`) and, for recovery entry, by the Section 6 caller `useAssessTaskFlow.transitionToStaleRecovery`, which invalidates the definition partials when recovery begins. Extracted from `useAssignmentDefinitionWizard.ts` (1,378 lines) as part of the required decomposition. Rationale: async orchestration belongs in feature hooks/modules, and recovery is an additional orchestrator entry on the same process — a second mode, not a duplicate flow.

- Decision: `new` (feature-local extraction)
- Owning module/path: `src/frontend/src/features/assignmentWizard/assignmentWizardOrchestrator.ts` (`resolveWizardEntryMode` + `useAssignmentWizardOrchestrator`, whose entry-mode taxonomy and stale-protection flow are documented in `@remarks` JSDoc)
- Call-site rationale: SPEC-mandated single readable orchestration file. The pinned entry intent union (`{kind:'create'} | {kind:'update',definitionKey} | {kind:'explicit-reparse',definitionKey} | {kind:'recovery',definitionKey,onApprovalSuccess}`) refuses a null recovery key fail-fast; reparses send `forceReparse: true` with the existing ID-shape request and omit weighting patches; approval saves resend the captured `expectedDefinitionUpdatedAt`; a successful reparse sets `hasParsedTasks` and persists immediately (review cancel discards local state only, never starts an assessment). Shared mutation/error-mapping plumbing is reused from the feature-local `assignmentWizardMutation.ts`, and the form-initialization/baseline hook lives in `assignmentWizardFormInitialization.ts`, keeping the hook file (439 lines) and every module below the 500-line gate.
- Status: `Implemented` (Section 7)
...
3. Wizard entry-contract extension — explicit recovery entry intent (existing `definitionKey` + approval-success callback; never `mode="create"`), plus the chrome-free review-content component recorded simultaneously in `docs/developer/frontend/frontend-modal-patterns.md` §3.4. Status: both `Implemented` — the review-content component landed in Section 5 and the recovery entry intent (plus the create/update/explicit-reparse intents) in Section 7 (`assignmentWizardOrchestrator.ts`).
