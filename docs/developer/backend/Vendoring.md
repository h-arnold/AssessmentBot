...
Previously, BeerCSS was installed via npm and vendored into HtmlService partials under `src/AdminSheet/UI/vendor/beercss/`. The `src/AdminSheet` directory and all its contents (including the vendored BeerCSS files) have been fully removed from the repository.
