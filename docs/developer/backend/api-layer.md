...
- `upsertAssignmentDefinition` — creates or updates a full assignment definition and synchronised registry partial.
  ...
  Optional request fields: `definitionKey`, `alternateTitles`, `alternateTopics`, `documentType`, `assignmentWeighting`, `taskWeightings`, `forceReparse`, and `expectedDefinitionUpdatedAt`.
  Validation split: the transport helper enforces request shape, recovery control-field shapes (`forceReparse` boolean; `expectedDefinitionUpdatedAt` a strict ISO datetime string with timezone when supplied non-null, while omission and explicit `null` retain create-time behaviour), safe-identifier rules for `definitionKey`, `taskWeightings[].taskId`, `referenceDocumentId`, `templateDocumentId`, and URL-extracted document IDs, and structural `taskWeightings` shape; the controller owns topic membership, duplicate-tuple rejection, numeric weighting rules, document-type rules, task-ID matching, and persistence semantics.
