...
- `src/AdminSheet/RequestHandlers/LLMRequestManager.js` — fixed the request generation and caching logic; ensured the cache is used and updated consistently.
- `src/AdminSheet/RequestHandlers/CacheManager.js` — stable cache key generation and a clearer cache put/get/serialize pipeline.
- `src/AdminSheet/y_controllers/InitController.js` — improved per-user initialisation & script authorisation checks; ensure the Admin Sheet and Assessment Record flows are distinct.
- `src/AdminSheet/Utils/ScriptAppManager.js` — encapsulated ScriptApp interactions and the per-user authorisation helpers used by `InitController`.
- `src/AdminSheet/DbManager/DbManager.js` — JsonDbApp wrapper and helpers for persisting assignment records.
- `src/AdminSheet/y_controllers/ABClassController.js` — added `persistAssignmentRun()` to write full JSON to `assign_full_...` collections and update partial records in the ABClass instance.
- `src/AssessmentRecordTemplate/menus/init.js` — fixed the template initialisation call to the correct library entrypoint and ensured menu creation is handled in the template context (per-user).
- `src/AdminSheet/zz_main.js` — added public helpers used by the assessment record template to check authorisation, create menus and revoke auth.
- `tests/requestHandlers/assignmentPhase3.test.js` and related tests — updated/added tests to verify caching behaviour, successful caching after retries, and the request generation paths.

## ⬆️ Updating to v0.7.5
