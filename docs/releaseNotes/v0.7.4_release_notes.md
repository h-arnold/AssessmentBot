...
- `src/AdminSheet/Utils/TriggerController.js` — centralised trigger creation and enforcement of the required scopes via `ScriptApp.requireScopes`.
- `src/AdminSheet/Utils/ScriptAppManager.js` — encapsulates ScriptApp interactions and exposes helpers for checking and handling authorisation.
- `src/AdminSheet/y_controllers/InitController.js` — updated to use ScriptAppManager and TriggerController; improved first-run flow and menu creation logic.
- `src/AdminSheet/ConfigurationManager/*` — removal of legacy `scriptAuthorised` configuration and schema updates.
- `src/AssessmentRecordTemplate/appsscript.json` and `src/AdminSheet/appsscript.json` — manifest scope/service alignment.
...
- During first-run or first authorisation, users may be prompted to grant the required scopes — follow the on-screen flow to grant permissions.

## 📦 Download

Get the updated files here: [Assessment Bot v0.7.4 Release Files](https://drive.google.com/drive/folders/1Aoj_7wN4buMyTEGL_GHtwv3LSNF2aAuk?usp=drive_link)
