...
## 🚨 Breaking change reminder from v0.7.6

- Assignment Definitions moved entirely into JsonDbApp and the ScriptProperties registry was emptied in v0.7.6. If you have not already completed the upgrade steps described there, resolve the state described in [the v0.7.6 release notes](v0.7.6_release_notes.md) before installing this patch.
...
- [src/AdminSheet/Utils/Utils.js](../../src/AdminSheet/Utils/Utils.js) — added trimming, warning logging, and a Slides export whitelist inside `isValidUrl` so we can surface the original error while still accepting the URL.
- [src/AdminSheet/Utils/TriggerController.js](../../src/AdminSheet/Utils/TriggerController.js) plus both `appsscript.json` manifests — switched Classroom scopes to the readonly variants and documented the scope-sync process.
- [docs/developer/backend/AssessmentFlow.md](../developer/backend/AssessmentFlow.md) — described the definition lifecycle and the rationale for the freshness re-check.
- [tests/utils/isValidUrl.test.js](../../tests/utils/isValidUrl.test.js) — new Vitest suite that verifies export URLs are accepted and demonstrates the warning/logging behaviour.

## ⬆️ Updating to v0.7.7
